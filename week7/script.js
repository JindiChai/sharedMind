...
const inputBox = document.createElement('emoji-picker');
...
document.getElementById('input_image_prompt').addEventListener("keyup", function (event) {
    if (event.key === "Enter") {
        askForPicture(document.getElementById('input_image_prompt').value);
        document.getElementById('input_image_prompt').value = "";
    }
});

// Add event listener to the input box
inputBox.addEventListener('emoji-click', function (event) {
    // Check if the Enter key is pressed


        const inputValue = event.detail.emoji.unicode;  
...
function subscribeToData(folder) {
    ...
    const backgroundRef = ref(db, appName + '/backgroundImage');
    onValue(backgroundRef, (snapshot) => {
        const backgroundImage = snapshot.val();
        if (backgroundImage && backgroundImage.imageUrl) {
            document.body.style.backgroundImage = "url(" + backgroundImage.imageUrl + ")";
...
authDiv.style.backgroundColor = "#d7a7e4";
...
// Modify Firebase database structure
function addBackgroundImageToFirebase(imageUrl) {
    const dbRef = ref(db, appName + '/backgroundImage');
    set(dbRef, { imageUrl: imageUrl });
}
...
async function askForPicture(p_prompt) {
    ...
    try {
        ...
        if (proxy_said.output.length == 0) {
            ...
            // Set background image for current client
            document.body.style.backgroundImage = "url(" + proxy_said.output[0] + ")";
            document.body.style.backgroundSize = "cover";
            document.body.style.backgroundRepeat = "no-repeat";

            // Save background image URL to Firebase
            addBackgroundImageToFirebase(proxy_said.output[0]);
        }
    } catch (error) {
